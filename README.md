...
5. Build the analyzer, conforming to the [Analyzer interface specification](https://github.com/exercism/docs/blob/main/building/tooling/analyzers/interface.md).
...
Once you're happy with your analyzer, [open an issue on the exercism/exercism repo](https://github.com/exercism/exercism/issues/new?title=%5BTRACK%5D+Request+Analyzer+Repository) to request an official analyzer repository for your track.

# Exercism TRACK_NAME_HERE Analyzer

The Docker image to automatically analyze TRACK_NAME_HERE solutions submitted to [Exercism].
...
To analyze an using the Docker image, do the following:
...
These are [golden tests][golden] that compare the `analysis.json` and `mapping.json` generated by running the current state of the code against the "known good" `tests/<test-name>/analysis.json`. All files created during the test run itself are discarded.
